...
# ==========================================
# 0. TỐI ƯU ĐƯỜNG DẪN CHO EXE
# ==========================================
def resource_path(relative_path):
    """ Hàm này giúp EXE tìm thấy file trong thư mục tạm của hệ thống """
    try:
...
# ==========================================
# 2. KIẾN TRÚC MẠNG (GIỮ NGUYÊN LOGIC)
# ==========================================
...
# ==========================================
# 3. TỐI ƯU HÓA NẠP TÀI NGUYÊN (CACHE)
# ==========================================
@st.cache_resource
def load_system_resources():
    """ Load model và metadata 1 lần duy nhất để tăng tốc """
    model = RamanResNet(num_targets=4)
    try:
        # Chỉ nạp tệp mô hình được chỉ định
        model.load_state_dict(torch.load(MODEL_PATH, map_location='cpu'))
    except Exception as e:
        st.error(f"Lỗi nạp mô hình tại {MODEL_PATH}: {e}")
    model.eval()
...
# Nạp sẵn tài nguyên ngay khi khởi động
